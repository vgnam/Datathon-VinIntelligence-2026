...
print("="*70)
print("BASELINE++ MINIMAL: BASE + 4-FEATURE RESIDUAL")
print("="*70)
...
# ============================================
# 1. BASE = Recent seasonal mean (KHONG traffic adjustment)
# ============================================
...
# ============================================
# 2. RESIDUAL voi chi 4 features quan trong nhat
# ============================================
# Chi chon 4 feature: chu ky ngay, Tet, promo, traffic
resid_features = ['day_cos', 'day_sin', 'is_tet_period', 'promo_seasonal_prob']
...
# Ridge voi alpha RAT LON de tranh overfit
best_mae_rev = np.inf
best_ridge_rev = None
best_mae_cogs = np.inf
best_ridge_cogs = None

print("\nGrid search Ridge (heavy regularization)...")
for alpha in [1000, 5000, 10000, 50000, 100000, 500000]:
    ridge_rev = Ridge(alpha=alpha)
    ridge_cogs = Ridge(alpha=alpha)
    ridge_rev.fit(X_tr, y_tr_rev)
    ridge_cogs.fit(X_tr, y_tr_cogs)
    
    pred_rev = train.loc[val_mask, 'base_rev'] + ridge_rev.predict(X_val)
    pred_cogs = train.loc[val_mask, 'base_cogs'] + ridge_cogs.predict(X_val)
    ...
    if mae_rev < best_mae_rev:
        ...
        best_ridge_rev = ridge_rev
    if mae_cogs < best_mae_cogs:
        best_mae_cogs = mae_cogs
        best_ridge_cogs = ridge_cogs

print(f"Best Rev: alpha={best_ridge_rev.alpha}")
print(f"Best COGS: alpha={best_ridge_cogs.alpha}")

# ============================================
# 3. VALIDATION
# ============================================
val_pred_rev = train.loc[val_mask, 'base_rev'] + best_ridge_rev.predict(X_val)
val_pred_cogs = train.loc[val_mask, 'base_cogs'] + best_ridge_cogs.predict(X_val)
...
# 4. FULL RETRAIN & TEST
# ============================================
ridge_rev_f = Ridge(alpha=best_ridge_rev.alpha)
ridge_cogs_f = Ridge(alpha=best_ridge_cogs.alpha)
ridge_rev_f.fit(train[resid_features], train['resid_rev'])
ridge_cogs_f.fit(train[resid_features], train['resid_cogs'])

test['pred_rev'] = test['base_rev'] + ridge_rev_f.predict(test[resid_features])
test['pred_cogs'] = test['base_cogs'] + ridge_cogs_f.predict(test[resid_features])
...
}).to_csv(out_dir / 'baseline_plus_plus_minimal_submission.csv', index=False)
print(f"\nSaved: {out_dir / 'baseline_plus_plus_minimal_submission.csv'}")

coef_rev = pd.DataFrame({'Feature': resid_features, 'Coef': ridge_rev_f.coef_}).sort_values('Coef', key=abs, ascending=False)
print("\nResidual coefficients (Revenue):")
