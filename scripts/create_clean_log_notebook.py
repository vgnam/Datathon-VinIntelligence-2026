...
cells.append(("markdown", """# Baseline+ with Cleaned Features + Log Transform

This notebook trains ML models (RF, LightGBM, XGBoost, ExtraTrees) using:
- **22 cleaned features**: `output/train_features_clean.csv`
- **Log transform** on Revenue and COGS targets
- Inverse transform: `pred = expm1(pred_log) - EPS`
"""))

cells.append(("markdown", "## 1 — Imports & Config"))
...
assert TRAIN_TARGET.exists(),   f'Missing {TRAIN_TARGET}'
"""))

cells.append(("markdown", "## 2 — Load Data"))

cells.append(("code", """train_feat = pd.read_csv(TRAIN_FEATURES, parse_dates=['Date']).sort_values('Date').reset_index(drop=True)
...
cells.append(("markdown", "## 3 — Log Transform Target"))

cells.append(("code", """EPS = 1.0
...
cells.append(("markdown", "## 4 — Train/Validation Split"))

cells.append(("code", """X = train.drop(columns=['Date', 'Revenue', 'COGS', 'Revenue_log', 'COGS_log']).copy()
...
cells.append(("markdown", "## 5 — Metrics & Helpers"))

cells.append(("code", """def mape(actual, pred):
...
cells.append(("markdown", "## 6 — Random Forest (Log Target)"))
...
for params in ParameterGrid(rf_grid):
    ...
    pred = pd.DataFrame(pred_raw, columns=['Revenue', 'COGS'])
    score = (mape(y_val_raw['Revenue'], pred['Revenue']) + mape(y_val_raw['COGS'], pred['COGS'])) / 2
    if score < best_score:
        best_score = score
        best_params = params
        best_model = model
...
results.append({'target':'Revenue','model':'RandomForest_log',
...
results.append({'target':'COGS','model':'RandomForest_log',
...
cells.append(("markdown", "## 7 — LightGBM (Log Target)"))
...
cells.append(("code", """try:
    ...
    results.append({'target':'Revenue','model':'LightGBM_log',
    ...
    results.append({'target':'COGS','model':'LightGBM_log',
...
cells.append(("markdown", "## 8 — XGBoost (Log Target)"))
...
cells.append(("code", """try:
    ...
    results.append({'target':'Revenue','model':'XGBoost_log',
        'MAPE':mape(y_val_raw['Revenue'],val_pred_rev),'SMAPE':smape(y_val_raw['Revenue'],val_pred_rev),
        'R2':r2(y_val_raw['Revenue'],val_pred_rev),'SSE':sse(y_val_raw['Revenue'],val_pred_rev)})
    results.append({'target':'COGS','model':'XGBoost_log',
        'MAPE':mape(y_val_raw['COGS'],val_pred_cogs),'SMAPE':smape(y_val_raw['COGS'],val_pred_cogs),
        'R2':r2(y_val_raw['COGS'],val_pred_cogs),'SSE':sse(y_val_raw['COGS'],val_pred_cogs)})

    xgb_rev_full = xgb.train({**xgb_base_params, **best_p_rev},
        xgb.DMatrix(X_full, label=y_log['Revenue_log']),
        num_boost_round=getattr(xgb_rev, 'best_iteration', 800), verbose_eval=False)
    xgb_cogs_full = xgb.train({**xgb_base_params, **best_p_cogs},
        xgb.DMatrix(X_full, label=y_log['COGS_log']),
        num_boost_round=getattr(xgb_cogs, 'best_iteration', 800), verbose_eval=False)
...
cells.append(("markdown", "## 9 — ExtraTrees (Log Target)"))

cells.append(("code", """from sklearn.ensemble import ExtraTreesRegressor
...
results.append({'target':'Revenue','model':'ExtraTrees_log',
...
results.append({'target':'COGS','model':'ExtraTrees_log',
...
cells.append(("markdown", "## 10 — Summary"))

cells.append(("code", """summary_df = pd.DataFrame(results)
summary_df = summary_df.sort_values(['target','MAPE']).reset_index(drop=True)
print(summary_df.to_string(index=False))
"""))

cells.append(("markdown", "## 11 — Save Submissions"))

cells.append(("code", """out_dir = Path(r'D:\\Datathon-2026\\data_cleaned\\forecast')
out_dir.mkdir(parents=True, exist_ok=True)

for name, pred_df in [('rf_clean_log', rf_test_pred), ('lgb_clean_log', lgb_test_pred),
                       ('xgb_clean_log', xgb_test_pred), ('et_clean_log', et_test_pred)]:
...
create_notebook(cells, r'D:\Datathon-2026\data\baseline_clean_log.ipynb')
print('\nDone creating baseline_clean_log.ipynb')
