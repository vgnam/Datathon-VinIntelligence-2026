...
cells.append(("markdown", """# Baseline+ with Balanced Features + Log Transform

This notebook trains ML models (RF, LightGBM, XGBoost, ExtraTrees, Ensemble) using:
- **31 balanced features**: `output/train_features_balanced.csv`
- **Log transform** on Revenue and COGS targets: `Revenue_log = log1p(Revenue + 1)`
- Inverse transform: `pred = expm1(pred_log) - 1`
"""))
...
cells.append(("markdown", "## 10 — Ensemble (Inverse MAPE weights)"))

cells.append(("code", """def mape_np(actual, pred):
    actual = np.asarray(actual, dtype=float)
    pred = np.asarray(pred, dtype=float)
    eps = 1e-9
    return float(np.mean(np.abs(actual - pred) / (np.abs(actual) + eps)))
...
cells.append(("markdown", "## 11 — Summary"))
...
cells.append(("markdown", "## 12 — Save Submissions"))
...
create_notebook(cells, r'D:\Datathon-2026\data\baseline_balanced_log.ipynb')
print('\nDone creating baseline_balanced_log.ipynb')
